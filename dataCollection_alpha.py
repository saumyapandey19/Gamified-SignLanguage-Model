...
print(f"[DEBUG] Label: '{label}', Hand Type: '{hand_type}'")

# Validate input
if not label or hand_type not in ["left", "right"]:
    print("[ERROR] Invalid input. Label must not be empty and hand type must be 'left' or 'right'.")
    exit()
...
while True:
    ...
    if results.multi_hand_landmarks:
        ...
        # Square bounding box centered around the hand
        ...
        # Ensure consistent size with white background padding
        hand_crop_resized = cv2.resize(hand_crop, (IMAGE_SIZE, IMAGE_SIZE))
        hand_final = np.ones((IMAGE_SIZE, IMAGE_SIZE, 3), dtype=np.uint8) * 255  # white background
        crop_h, crop_w = hand_crop_resized.shape[:2]
        hand_final[:crop_h, :crop_w] = hand_crop_resized
