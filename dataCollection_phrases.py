...
# Input from user
label = input("Enter phrase label (e.g., 'hello', 'thank you'): ").strip().lower()
hand_type = input("Enter hand type (left/right): ").strip().lower()

# Validation
if label not in allowed_phrases or hand_type not in ["left", "right"]:
    print("[ERROR] Invalid input.")
    print(f"→ Valid phrases: {', '.join(allowed_phrases)}")
    print("→ Hand type must be 'left' or 'right'.")
    exit()

# Format folder name
safe_label = label.replace(" ", "_")  # handle phrases with spaces
folder_name = f"{safe_label}_{hand_type}"
